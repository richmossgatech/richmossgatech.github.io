...
To interact with the visualization, the user can do the following: change the filters to display the related nodes and edges, change the group information to get more information about the cluster, click on a player to see that players statistics from the corresponding season or press enter to select a random visible player node, and use the right arrow key to navigate from the selected player along an edge to a similar player.  

First, the filters are used to control which nodes and edges are shown.  The season filter shows all player nodes from the NBA season selected, along with the corresponding edges which connect them.  The edges filter is used to adjust the level of similarity shown in the visualization.  By increasing the edge value in this dropdown, only edges between players with a higher edge weight will be shown.  The show/do not show unconnected nodes filter will show or hide any nodes not connected by an edge to another node based on the currently applied filters.  Finally, the team and position filters will filter players based on their team, or position.  Of note, all filters can be stacked, allowing for more specific conditions to be imposed on the graph. 

Second, the nodes (players) are colored based on the group they belong to.  To gain more information about the shared features of a cluster, select the group of interest from the dropdown in the bottom right. Selecting a group will display a name given to the group, the player positions which generally appear in that group, a description of some shared statistics of players in the group, and a pair of prominent players in that group.  

Third, clicking on a node will outline the node in a black ring, and bring up that player's per game pace adjusted statistics from that season. When running the visualization locally, the updating of the visualization can make clicking difficult. In this case, the user can also press enter to select a random node currently visible based on the applied criteria.  By hovering over or selecting a node, the players name and season will also be visible on the graph. 

Finally, pressing the right arrow key allows for the user to get an understanding of player connections within the graph. Once a node is selected, pressing the right arrow key will select a new node connected to the current node based on the applied filters.  This new node will be circled in black, and the player stats box will update.  To move around the graph only between players with more similarities, simply adjust the edge filter. 

The visualization can be reset at any time simply by refreshing the page.
